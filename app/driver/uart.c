...
extern uint32 pos_start;//read
extern uint32 pos_current;//write
extern uint8 *hal_RxBuffer;

typedef void (*pfMasterMCU_ReciveData)( uint8 *pData,int32 dataLen,int8 isFinish );
 ...
 * Returns      : NONE
*******************************************************************************/
extern int32 ICACHE_FLASH_ATTR Local_UartRx_Timer();
LOCAL void
uart0_rx_intr_handler(void *para)
{
    ...
    */
    ...
    else if(UART_RXFIFO_FULL_INT_ST == (READ_PERI_REG(UART_INT_ST(uart_no)) & UART_RXFIFO_FULL_INT_ST))
    {
        ...
        // if(available_len < fifo_len)
        // {
        //     fifo_len =available_len;
        // }

        for(buf_idx=0;buf_idx<fifo_len;buf_idx++)//接收串口数据
        {
            temp = READ_PERI_REG(UART_FIFO(UART0)) & 0xFF;
            if( PF_ReceiveDataformMCU )
            {
                PF_ReceiveDataformMCU( &temp,1,buf_idx==(fifo_len-1) );
            }
            // if(NULL != hal_RxBuffer)
            // {
            //     hal_RxBuffer[(pos_current+buf_idx) & HAL_BUF_MASK] = temp;
            // }
        }
        // pos_current += fifo_len;
        WRITE_PERI_REG(UART_INT_CLR(UART0), UART_RXFIFO_FULL_INT_CLR|UART_RXFIFO_TOUT_INT_CLR);
        uart_rx_intr_enable(UART0);
        // system_os_post( TASKID_MAINTASK, SIG_UART, 0 );
