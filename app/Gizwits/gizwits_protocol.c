...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLiteSwitch)
    {
        info->event[info->num] = EVENT_LiteSwitch;
        info->num++;
        dataPoints->valueLiteSwitch = gizStandardDecompressionValue(LiteSwitch_BYTEOFFSET,LiteSwitch_BITOFFSET,LiteSwitch_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagGate)
    {
        info->event[info->num] = EVENT_Gate;
        info->num++;
        dataPoints->valueGate = gizStandardDecompressionValue(Gate_BYTEOFFSET,Gate_BITOFFSET,Gate_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLiteSwitch != cur->valueLiteSwitch)
    {
        GIZWITS_LOG("valueLiteSwitch Changed\n");
        ret = 1;
    }
    if(last->valueGate != cur->valueGate)
    {
        GIZWITS_LOG("valueGate Changed\n");
        ret = 1;
    }
    if(last->valueGateSta != cur->valueGateSta)
    {
        GIZWITS_LOG("valueGateSta Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(LiteSwitch_BYTEOFFSET,LiteSwitch_BITOFFSET,LiteSwitch_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLiteSwitch);
    if(dataPoints->valueGate >= Gate_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueGate Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(Gate_BYTEOFFSET,Gate_BITOFFSET,Gate_LEN,(uint8_t *)devStatusPtr,dataPoints->valueGate);  
    }
    gizStandardCompressValue(GateSta_BYTEOFFSET,GateSta_BITOFFSET,GateSta_LEN,(uint8_t *)devStatusPtr,dataPoints->valueGateSta);
